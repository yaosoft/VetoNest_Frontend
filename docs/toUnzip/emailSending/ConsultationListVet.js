  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (rep?.success) {
        ...
        try {
          const petOwnerUserId = rep.petOwnerUserId ?? selected.carnetAnimal?.userId;
        ...
        // Send email to pet owner using data returned directly from the backend
        try {
          if (rep.ownerEmail) {
            const [date, time] = (rep.startingDatetime ?? '').split(' ');
            await sendEmail({
